...
const ProjectDetailSection = () => {
          ...
          projects.forEach((_, index) => {
            ...
            if (card) {
              if (index === 0) {
                ...
                // 각 카드가 정확히 한 페이지(100vh)에 걸쳐 올라오도록
                const pageIndex = index - 1; // 0, 1, 2
                const totalScrollPages = projects.length - 1; // 3
                
                // 각 페이지의 시작과 끝 지점
                const pageStart = pageIndex / totalScrollPages;
                const pageEnd = (pageIndex + 1) / totalScrollPages;
                
                if (scrollProgress < pageStart) {
                  // 아직 카드가 나타나지 않음
                  gsap.set(card, { y: '100%' });
                } else if (scrollProgress >= pageStart && scrollProgress <= pageEnd) {
                  // 카드가 천천히 올라오는 구간 (한 페이지 전체에 걸쳐)
                  const pageProgress = (scrollProgress - pageStart) / (pageEnd - pageStart);
                  const translateY = 100 - (pageProgress * 100);
  ...
  return (
    ...
    >
      ...
      {/* 오른쪽 프로젝트 영역 */}
      <div className="absolute top-0 right-0 w-1/2 h-screen overflow-hidden bg-gray-50">
        {projects.map((project, index) => (
          <div 
            key={project.id}
            className={`project-card-${index} absolute inset-0 w-full h-full flex items-center justify-center p-8`}
            style={{ 
              transform: 'translateY(100%)',
              zIndex: projects.length - index // 뒤의 카드가 더 높은 z-index
            }}
          >
